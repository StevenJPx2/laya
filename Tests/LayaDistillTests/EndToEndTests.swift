...
final class EndToEndTests: XCTestCase {
    ...
    func testTinyStudentEndToEnd() async throws {
        ...
        XCTAssertEqual(artifact.features.scheme, "hashed-ngram-v1")
        ...
        XCTAssertEqual(gold.layaRaw?.accuracy, 1, "the fake teacher is perfect on gold by construction")
        XCTAssertEqual(gold.layaRaw?.examples, 18)

        let classifier = Classifier(artifact: artifact)
        XCTAssertEqual(try classifier.predict(.object([("request", .string("wipe the backups"))])).label, "deny")
        XCTAssertEqual(try classifier.predict(.object([("request", .string("list files")), ("note", .string("routine"))])).label, "allow")
        XCTAssertThrowsError(try classifier.predict(.object([("command", .string("x"))])))

        let again = try Workflow.evaluate(classifier, examples: examples, labels: labels)
    ...
    func testRegistryServesClassifyWithoutLaya() async throws {
        ...
        let directory = try Fixture.temporaryDirectory()
        try Workflow.train(spec: spec, examples: examples, labels: try await Fixture.label(spec, examples))
    ...
    func testTamperedArtifactIsRejected() async throws {
        ...
        try Workflow.train(spec: spec, examples: examples, labels: try await Fixture.label(spec, examples)).save(to: url)
    ...
    func testTrainingNeedsConfidentLabels() async throws {
        ...
        XCTAssertThrowsError(try Workflow.train(spec: spec, examples: examples, labels: labels)) { error in
    ...
    func testL2IsSelectedByCrossValidationOnTrainOnly() async throws {
        ...
        let artifact = try Workflow.train(spec: spec, examples: examples, labels: try await Fixture.label(spec, examples))
